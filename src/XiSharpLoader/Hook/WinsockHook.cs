...
namespace XiSharpLoader.Hook
{
    internal class WinsockHook
    {
        ...
        public void Hook()
        {
            ...
            if (!NativeMethods.VirtualProtectEx(_handle, _apiAdd, new UIntPtr((uint)_trampoline.Length), (uint)NativeMethods.MemoryProtection.ExecuteReadWrite, out uint oldProtection))
            {
                HandleError();
            }

            if (!NativeMethods.WriteProcessMemory(_handle, _apiAdd, _trampoline, _trampoline.Length, out IntPtr lpNumberOfBytesRead))
            {
                HandleError();
            }
        ...
        private void Check()
        {
            if (!((IEnumerable<ProcessModule>)_process.Modules).Any(module => module.ModuleName.ToLower() == "ws2_32.dll"))
            {
